...
- **The itinerary (calendar details: flights, hotels, confirmation numbers) is private data** stored at `trips/japan-2026/config/itinerary` (`{defaultCity, stays, itin}`; `itin[day].stay` is a string key into `stays`; `itin[day].more` is an optional array of extra same-day events shaped like `main`). It is NOT in `index.html` — the client hydrates it after sign-in (`hydrateItin()`), caches it in localStorage (`_itin`), and shows a blurred "Sign in to view calendar" overlay (`#calLock`) when signed out. Never hardcode itinerary details back into the repo.
...
- `worker/` is a **Cloudflare Email Worker** (`japanclan-mail`), separate from the static site (excluded from the Pages deploy). Emails to `*@trips.haaans.com` route to it; it parses ALL bookings in an email (multi-VEVENT ICS fast-path, else one Gemini structured-output call returning an events array) and merges them into the Firestore itinerary doc via REST with a service-account JWT (IAM bypasses security rules — the rules are untouched).
- Events group into trips **by date** (`resolveTripByDate`): whichever configured trip window contains the event's start date wins; sender default only breaks ties. Same-day events coexist — flights/trains take `itin[day].main`, everything else goes in `itin[day].more` (time-sorted array of main-shaped objects; the app hydrates and renders it). Re-sent bookings (same confirmation/title) update in place. Only overlapping *different* hotel stays still conflict → pending.
- Senders drive everything by email reply: YES/NO (confirm low-confidence proposals), UNDO (snapshot-based revert, precondition-guarded), STATUS, HELP (`src/commands.js`). Low-confidence parses (< `llm.autoThreshold`, default 0.75) are proposed, never auto-added. Loop guards drop automated mail (Auto-Submitted/mailer-daemon/own domain); per-sender daily cap via `options.maxPerSenderPerDay` (default 30).
